...
# Default-Stop:      0 1 6
# Short-Description: ksuid
# Description:       K-Sortable Globally Unique IDs for Java
...
cd "$(dirname "$0")" || exit 1
[[ -z "$jarfile" ]] && jarfile=$(pwd)/$(basename "$0").jar
while [[ -L "$jarfile" ]]; do
  ...
  jarfile=$(readlink "$jarfile").jar
  cd "$(dirname "$jarfile")" || exit 1
  jarfile=$(pwd)/$(basename "$jarfile").jar
done
